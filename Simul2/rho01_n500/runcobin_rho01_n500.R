...
dataall = readRDS("data/betarec_rho01_n500.rds")
...
nsave = 5000
nthin = 1
fit <- cobin::spcobinreg(y ~ X, df, coords = data$coords_train, NNGP = F,
                         priors = list(phi_lb = data$phi_spatial, phi_ub = data$phi_spatial,
                                       beta_intercept_scale = 100,lambda_grid = 1:70,
...
for(itest in 1:data$ntest){
  ...
  for(isave in 1:nsave){
    logpostpred_mcmc[isave] = cobin::dcobin(data$y_test[itest], pred_out$linpred_test_save[isave,itest], fit$post_save[isave,"lambda"], log = TRUE)
  }
...
saveRDS(summary_list, file = paste0("rho01_n500/cobinregfit_", slurm_id, ".rds"))
