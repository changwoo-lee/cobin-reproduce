...
# Install FNN if not already installed
...
# Choose target index
target_index <- which(df$comid == 22721231)
target_index

# Run nearest neighbor search
neighbors <- get.knnx(data = df[, c("easting", "northing")],
                      query = df[target_index, c("easting", "northing")],
                      k = 11)  # 11 includes the point itself

# Extract neighbor indices (excluding the point itself)
neighbor_indices <- neighbors$nn.index[1, -1]
neighbor_distances <- neighbors$nn.dist[1, -1]

# Print result
mean(df$MMI_BENT[neighbor_indices])

df[target_index,]

###########################################
target_index <- which(df$comid == 9201925)
target_index
...
###########################################
target_index <- which(df$comid == 22845861)
target_index
