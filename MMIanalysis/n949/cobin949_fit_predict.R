...
if(RUN){
  ...
  saveRDS(fit_cobin1, file = "res/fit_cobin1_n949.rds")
  set.seed(11)
  fit_cobin2 = spcobinreg(MMI_BENT ~ agkffact_logstd + bfi_logstd +
                          ...
                          nngp.control = list(n.neighbors = 15, ord = o),
                          nburn = 1000, nsave = 5000)
  saveRDS(fit_cobin2, file = "res/fit_cobin2_n949.rds")
  set.seed(111)
  fit_cobin3 = spcobinreg(MMI_BENT ~ agkffact_logstd + bfi_logstd +
                          ...
                          nngp.control = list(n.neighbors = 15, ord = o),
                          nburn = 1000, nsave = 5000)
  saveRDS(fit_cobin3, file = "res/fit_cobin3_n949.rds")
}else{
  fit_cobin1 = readRDS("res/fit_cobin1_n949.rds")
  fit_cobin2 = readRDS("res/fit_cobin2_n949.rds")
  fit_cobin3 = readRDS("res/fit_cobin3_n949.rds")
}
...
saveRDS(list(
  ...
  file = "res/summary_cobin_n949.rds")
...
fit_cobin1$spNNGPfit$p.beta.samples = fit_cobin1$post_save[,1:ncol(fit_cobin1$X)]
# order important
fit_cobin1$spNNGPfit$p.theta.samples = fit_cobin1$post_save[,c("sigma.sq", "phi")]
fit_cobin1$spNNGPfit$p.w.samples = t(as.matrix(fit_cobin1$post_u_save))

# predict using spNNGP; takes some time
pred_cobin1 <- predict(fit_cobin1$spNNGPfit, 
                       X.0 = Xnew, 
                       coords.0 = cbind(df4$easting,df4$northing),
                       sub.sample = list(start = 1, end = fit_cobin1$nsave, thin = 1))
...
pred_cobin1$p.w.0[is.nan(pred_cobin1$p.w.0)] = NA
pred_cobin1$p.y.0[is.nan(pred_cobin1$p.y.0)] = NA

wnew_hat_cobin = rowMeans(pred_cobin1$p.w.0, na.rm = TRUE)
wnew_sd_cobin = apply(pred_cobin1$p.w.0, 1, sd, na.rm = TRUE)

munew_hat_cobin = rowMeans(cobin::bftprime(pred_cobin1$p.y.0), na.rm = TRUE)
munew_sd_cobin = apply(cobin::bftprime(pred_cobin1$p.y.0), 1, sd, na.rm = TRUE)



drawdf4_cobin = data.frame(
  x = transformed_coords4[,1],
  y = transformed_coords4[,2],
  urbmdhi = log2(1+df4$urbmdhi),
  wnew_hat = wnew_hat_cobin,
  wnew_sd = wnew_sd_cobin,
  munew_hat = munew_hat_cobin,
  munew_sd = munew_sd_cobin
)
saveRDS(drawdf4_cobin, "res/predict_cobin.rds")
